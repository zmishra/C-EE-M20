%Script: SIR Model for Infection (Part C)
%Zubin Mishra, 604644805
%This script simulates the process of spreading disease when a vaccine is
...
% For loop
for k = 1:tSteps
    s(k+1) = s(k)+deltat*(-beta(k)*i(k)*s(k));
    i(k+1) = i(k)+deltat*(beta(k)*i(k)*s(k)-gamma*i(k));
